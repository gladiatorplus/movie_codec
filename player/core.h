...
struct frame_info {
    ...
    int num_vsyncs;         // scheduled vsyncs, if using display-sync//计划的vsyncs（如果使用display sync）
};
 ...
 * of A/V sync.
 * /*请注意，回放可以随时暂停、停止等。暂停时，播放重新启动仍处于活动状态，因为即使暂停，
 * 也希望继续工作。区分这些状态的主要目的是正确地重新初始化A/V同步。
 */
...
typedef struct MPContext {
    ...
    //在MPContext销毁正常之前仍需要完成的异步任务数。这意味着异步talsk每次递减时都会调用mp_wakeup_core（）。
    // As using an atomic+wakeup would be racy, this is a normal integer, and
    // mp_dispatch_lock must be called to change it.
    //由于使用atomic+wakeup会很刺激，这是一个普通的整数，必须调用mp_dispatch_lock来更改它。
    ...
    //乘以opts->playback_speed得到总的音频或视频速度（通常为1.0，但可以通过同步代码设置）。
    ...
    ////改变了多少视频计时以使其与音频时间线匹配。仅用于状态行信息。
    double total_avsync_change;
    // A-V sync difference when last frame was displayed. Kept to display
    // the same value if the status line is updated at a time where no new
    // video frame is shown.
    //上一帧时的A-V同步差异显示。保留如果状态行在没有显示新视频帧的时候被更新，则显示相同的值。
    double last_av_difference;
    /* timestamp of video frame currently visible on screen
     * (or at least queued to be flipped by VO) */
    //屏幕上当前可见的视频帧的时间戳（或至少排队等待VO翻转）
    double video_pts;
    ...
    //某些计时函数上次读取当前时间的时间戳，以微秒为单位。用于将新的时间值转换为上次的增量。
    int64_t last_time;
    ...
    //在显示解码速度对实时播放速度太慢的警告后设置。用来避免多次显示。
