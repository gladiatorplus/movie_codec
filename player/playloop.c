...
//使播放循环运行。这可以从任何线程调用。如果从playloop本身内部调用，它将立即再次运行，而不是在下一个mp_wait_events（）中休眠。
...
//处理任何排队的输入，无论是用户输入，还是来自客户端API线程的请求。这还会重置与mp_wait_events（）一起使用的“wakeup”标志
...
void execute_queued_seek(struct MPContext *mpctx)
{
    if (mpctx->seek.type) {
        ...
        /*如果用户连续seek（按住箭头键），请尝试在执行另一个seek之前从一个位置完成显示帧（这可能导致显示不变）。*/
...
static void handle_loop_file(struct MPContext *mpctx)
{
        ...
        (opts->ab_loop[0] != MP_NOPTS_VALUE || opts->ab_loop[1] != MP_NOPTS_VALUE))
    {
        ...
        //假设execute_queued_seek（）在下一个音频/视频尝试解码或过滤之前发生。
        mpctx->stop_play = KEEP_PLAYING;
...
//执行VO窗口的强制刷新。这将从上一个视频中清除窗口。它还根据需要创建/销毁VO。
//它只尝试在确定需要或不需要窗口的情况下进行更改，或者如果设置了force参数（后者还决定是否清除现有窗口，因为无法知道是否已经执行了此操作）。
int handle_force_window(struct MPContext *mpctx, bool force)
{
    ...
    //如果我们处于空闲模式，或者文件加载已完成，则在文件加载过程中的某些阶段也会通过强制设置。
    ...
    //另一方面，如果一个视频轨迹被选中，但是没有视频被解码，那么就创建一个窗口。
...
//在实际开始播放之前，我们总是确保音频和视频缓冲区已填满。此代码处理同时启动它们。
//视频播放
