...
//给定一个由新行分隔的巨大字符串，尝试切断当前行上方的文本以保持该行可见，并切掉下面的文本以保持渲染性能。pos给出当前行（第一行为0）。
//“text”可以按原样返回，也可以释放它并返回新的分配。
//这只是一个启发-我们不能处理断线。
...
// recursion.
//选项属性桥。这是为了通过各种机制（包括命令行解析、配置文件、每个文件选项）设置选项来更新与它们相关联的状态。
//为此，它们必须通过属性层。（理想情况下，这将是另一种情况，而是使用每个选项的更改处理程序。）
//注意，属性选项bridge回避了这一点，因为我们可以得到无限递归。
int mp_on_set_option(void *ctx, struct m_config_option *co, void *data, int flags)
{
    ...
    //跳过mp_property_generic_选项（通常情况下），因为属性实现很简单，如果涉及非平凡标志（网桥会删除这些标志），
    //则会破坏一些模糊的功能，例如--profile和--include）。
...
/*在OSD上显示属性的默认方法列表。
*
*如果设置了osd_progbar，则会显示一个显示属性最小值/最大值之间的当前位置的条形图。在这种情况下，osd_msg只用于没有视频的终端输出；它将是一个与百分比一起显示的标签。
*/
