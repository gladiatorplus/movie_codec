...
*MPContext > mp_client_api.lock > mpv_handle.lock > * > mpv_handle.wakeup_lock
*
严格地说，MPContext没有锁，而是由MPContext.dispatch，它基本上在定义的点停止播放线程，
以便让客户端以同步方式访问它。由于MPContext代码访问客户机API，所以它位于锁层次结构的顶部。
*
*/
...
//测试所有客户端的“模糊”初始化。也就是说，自创建（或退出）以来，所有客户端至少调用了一次mpv_wait_event（）。
...
// Note: the caller has to deal with sporadic（零星的） wakeups.
...
static void mp_destroy_client(mpv_handle *ctx, bool terminate)
{
    ...
    //reserved_events等于尚未回复的异步请求数。为了避免试图回复已删除的客户机而导致崩溃，请阻塞，直到所有异步请求都得到处理。
    ...
    //请注意，即使num_clients==0，设置have_terminator也会使mpctx和核心线程保持活动状态。
    if (terminate) {
        ...
        //确保核心停止播放文件等。能够锁定调度队列需要核心线程仍然处于活动状态。
...
// Can be called on the core thread only. Idempotent.只能在核心线程上调用。幂等的。
...
static void *core_thread(void *p)
{
    ...
    // the last mpv_handle.
    //这实际上要等到所有客户机都离开之后，才能真正销毁mpctx。实际的破坏是由破坏最后一个 mpv_handle的东西完成的。
...
//该回复必须事先用reserve_reply保留（否则，请使用send_event（））
...
//异步运行命令。来电者有责任实际发送回复。这个助手只保存了所需样板的一小部分。
...
//如果有任何未完成的属性，请将ctx->cur嫒event设置为生成的属性更改事件。
static bool gen_property_change_event(struct mpv_handle *ctx)
{
    if (!ctx->mpctx->initialized)
        return false;
