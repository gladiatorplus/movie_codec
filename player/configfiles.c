...
void mp_write_watch_later_conf(struct MPContext *mpctx)
{
    ...
    //这允许我们递归地恢复目录等，它们的条目在第一次“播放”时就被扩展了。例如，如果“/a/b/c.mkv”是当前条目，那么如果用户执行“mpv/a”操作，则我们希望恢复此文件。
    //这将扩展到“/a”中的目录条目，如果“/a/a.mkv”不是第一个条目，则会播放该条目。
    //在这里，我们为“/a”和“/a/b”写简历条目。
    //（不幸的是，这将在resume中留下散乱的resume文件，因为显然它只从其中一个路径继续。）
    ...
    //最后，对于本地目录，我们为每个路径前缀写一个条目，这样用户就可以从任意目录恢复。这从第一个重定向开始（所有其他重定向都是进一步的前缀）。
...
//返回具有恢复配置的第一个文件。与散列播放列表文件或内容并为它们管理单独的恢复文件相比，
// 这更简单，而且还具有附加到播放列表中不干扰恢复的良好特性（尤其是当播放列表来自命令行时）。
