...
// not write to it.
//在不改变读取位置的情况下，最多提前读取len字节。从当前读取位置返回指向内部缓冲区的指针。
//最多可预读流最大缓冲区大小字节。该返回的缓冲区在下一个流调用时无效，您不能对其进行写入。
...
bool stream_skip(stream_t *s, int64_t len)
{
    ...
    if (len > 2 * STREAM_BUFFER_SIZE && s->seekable) {
        ...
        ////搜索到目标前1个字节-这是区分跳过EOF和跳过EOF的唯一方法。成功的查找完全没有意义，所以通过实际读取最后一个字节来进行测试。
