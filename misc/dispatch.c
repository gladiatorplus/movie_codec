...
//与mp_dispatch_enqueue（）类似，但在运行fn回调之后，队列代码将调用talloc_free（fn_data）。
//（回调本身可以很简单地做到这一点，但是使用相同的回调实现可以更容易地实现同步和异步请求。）
...
//如果队列在mp_dispatch_queue_process（）内，则使其在所有工作项运行完毕后立即返回，而不等待超时。如果被mp_dispatch_lock（）阻止，它不会提前返回。
// 如果队列不在mp_dispatch_queue_process（）内，
// 请使用超时值0对其进行下一次调用（如果需要以无争用方式从另一个线程唤醒主线程，这是一个有用的行为）。
